    ...
    async execute(interaction, client) {
        ...
        if (interaction.isButton() && interaction.customId.startsWith('ticket_')) {
            ...
            try {
                ...
                await interaction.reply({ content: `Ticket opened! Please check <#${ticketChannel.id}>.`, ephemeral: true });
                await ticketChannel.send({ content: `<@${interaction.user.id}>, your **${ticketType}** ticket has been created. Please describe your issue clearly and cleanly and a member of the Staff or Support team will be with you shortly.` });
            } catch (error) {
                console.error('[Ticket Fault]', error);
                const errorCode = error.code ? ` (Code: ${error.code})` : '';
                await interaction.reply({ content: `Failed to create ticket: ${error.message}${errorCode}`, ephemeral: true }).catch(() => {});
            }
            return;
        }
