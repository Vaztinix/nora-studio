...
class Logger {
    ...
    /**
     * Resolve the target logging channel ID for a specific category based on premium status.
     * Fallback to the main settings.loggingChannelId if not premium or if the split channel is not configured.
     */
    resolveLogChannelId(settings, category) {
        ...
        if (isPremium && settings.loggingChannels) {
